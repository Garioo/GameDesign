...
2. **Supabase project:** create one, then apply `supabase/schema.sql` to its database (SQL editor
   ...
   29. `migrate-event-recurrence.sql` — repeating calendar events (`repeat` daily / weekly / every 2 weeks / monthly, `repeat_until`, `skipped_dates` for single deleted occurrences) `repeat_weekends` (off = no Saturday/Sunday occurrences), an agenda checklist per event (`agenda`) ticked per occurrence in `calendar_agenda_checks`, and meeting notes per occurrence in `calendar_event_notes` — so each occurrence starts unticked and empty. Occurrences are computed client-side in `lib/calendarRecurrence.ts`. Apply after `migrate-event-attendees.sql`; the calendar shows a "needs a database update" error until it is applied.
