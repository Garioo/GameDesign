-- Adds an optional p_color parameter to save_board_stages so a board's own color (not just its
-- stages' colors) can be set on creation and changed afterward. Omitting it keeps today's
-- behavior: '#64748b' on create, unchanged on rename. Run once after migrate-custom-stages.sql.
begin;
create or replace function public.save_board_stages(
 p_project uuid,p_board uuid,p_expected jsonb,p_name text,p_stages jsonb,p_transfers jsonb default '{}',p_color text default null
...
begin
 ...
 if p_color is not null and p_color !~ '^#[0-9A-Fa-f]{6}$' then raise exception 'Choose a valid board color'; end if;
 ...
 insert into boards(project_id,name,color,position) select p_project,trim(p_name),coalesce(p_color,'#64748b'),coalesce(max(position),-1)+1 from boards where project_id=p_project returning id into target;
 end if;
 perform set_config('gantt.writing','yes',true);
 update boards set name=trim(p_name),color=coalesce(p_color,color) where id=target and project_id=p_project;
...
$$;
-- Adding a 7th parameter creates a second overload; drop the old 6-arg signature so PostgREST
-- doesn't see two RPCs named save_board_stages.
drop function if exists public.save_board_stages(uuid,uuid,jsonb,text,jsonb,jsonb);
revoke all on function public.save_board_stages(uuid,uuid,jsonb,text,jsonb,jsonb,text) from public,anon;
grant execute on function public.save_board_stages(uuid,uuid,jsonb,text,jsonb,jsonb,text) to authenticated;
